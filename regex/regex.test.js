...
//     let email = /^\w+([\-]?\w+)*@\w+([\.-]?\w+)*(\.\io)+$/;
//     if (emailAdress.match(email)) {
//         return true;
//     } else {
//         return false;
//     }
...
function capitalA(s){
       if (/[A-Z]/.test(s) && /[a-z]/.test(s) && s.slice(-1) == 'A'){
        return true;
       }
      return false;
}
...
function imagesSearcher(text){
    let arr = [];
     // Add your logic.
     if (/\.png/.test(text) || /\.jpg/.test(text) || /\.jpeg/.test(text)  ){
        const regex = /[a-zA-Z]+\.png/g;
        const regex2 = /[a-zA-Z]+\.jpg/g;
        const regex3 = /[a-zA-Z]+\.jpeg/g;
        let found = text.match(regex).join('')
        let found2 = text.match(regex2).join('')
        let found3 = text.match(regex3).join('')
        arr.push(found)
        arr.push(found2)
        arr.push(found3)
        return arr;
